...
def resolve_path(kind: str, cli_value: Optional[Path], manifest: Dict[str, Any], manifest_path: Path) -> Path:
    ...
    manifest_relative = manifest_path.parent / p
    if manifest_relative.exists():
        return manifest_relative

    legacy_repo_relative = p
    if legacy_repo_relative.exists():
        print(f"[WARN] legacy manifest path format detected for files.{kind}.path={rel}; prefer manifest-relative filenames such as {kind}.jsonl")
        return legacy_repo_relative

    manifest_parts = manifest_path.parent.parts
    max_overlap = min(len(manifest_parts), len(p.parts))
    for overlap in range(max_overlap, 0, -1):
        if tuple(manifest_parts[-overlap:]) != tuple(p.parts[:overlap]):
            continue
        base_dir = manifest_path.parent.parents[overlap - 1]
